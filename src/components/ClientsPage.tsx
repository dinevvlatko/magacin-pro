...
function addOrder(totals:ProductTotals,order:Order){
  ...
  totals.p15Packages+=order.qty15
  totals.p15Pieces+=order.qty15Pieces||0
...
function ClientDetails({summary,onClose}:{summary:ClientSummary;onClose:()=>void}){
  ...
  return <Modal title={client.name} onClose={onClose}><div className="client-detail"><div className="client-detail-contact"><span><MapPin size={16}/>{client.city}{client.address?` • ${client.address}`:''}</span><span>{client.contactPerson||'Нема контакт лице'} • {client.phone||'Нема телефон'}</span></div><div className="client-detail-metrics"><div><ShoppingBag/><span>Вкупно нарачки<strong>{orders.length}</strong></span></div><div><PackageCheck/><span>Земени од магацин<strong>{taken.length}</strong></span></div><div><Truck/><span>Последна нарачка<strong>{lastOrder?.date||'—'}</strong></span></div></div><section className="client-consumption"><div className="section-title"><div><h3>Потрошувачка по производ</h3><p>Основа за идна анализа на продажба и планирање залиха</p></div></div><div className="client-consumption-table"><div className="consumption-head"><span></span><b>Нарачано</b><b>Земено</b></div><ConsumptionRow label="0.25 L" ordered={formatPackages(orderedTotals.p025Packages,orderedTotals.p025Pieces)} taken={formatPackages(takenTotals.p025Packages,takenTotals.p025Pieces)}/><ConsumptionRow label="1.5 L BIB" ordered={formatPackages(orderedTotals.p15Packages,orderedTotals.p15Pieces)} taken={formatPackages(takenTotals.p15Packages,takenTotals.p15Pieces)}/><ConsumptionRow label="Флаери" ordered={`${orderedTotals.flyers} парчиња`} taken={`${takenTotals.flyers} парчиња`}/></div></section><section className="client-history"><div className="section-title"><div><h3>Историја на нарачки</h3><p>Кога и што има нарачано клиентот</p></div></div>{orders.length===0?<Empty text="Клиентот сè уште нема нарачки."/>:<div className="client-order-list">{orders.map(order=><article key={order.id}><div className="client-order-head"><div><span>{order.number}</span><strong>{order.date}</strong></div><span className={statusClass(order.status)}>{order.status}</span></div><div className="client-order-products"><span>0.25 L <b>{formatPackages(order.qty025,order.qty025Pieces||0)}</b></span><span>Гратис <b>{formatPackages(order.free025,order.free025Pieces||0)}</b></span><span>BIB <b>{formatPackages(order.qty15,order.qty15Pieces||0)}</b></span><span>Флаери <b>{order.flyers} пар.</b></span></div>{order.note&&<p>{order.note}</p>}</article>)}</div>}</section></div></Modal>
}
